...
void IC_Init(void)
{
	...
	TIM_TimeBaseInitStructure.TIM_Period = 65536 - 1;//设置ARR的值，最大上限接近1MHz,超过误差会很大，1Mz有1%的误差
	TIM_TimeBaseInitStructure.TIM_Prescaler = 72 - 1;//设置PSC的值，现支持的最低频率为15Hz,可通过增大PSC的值降低下限
	...
	TIM_ICIintSturcture.TIM_ICPolarity = TIM_ICPolarity_Rising;//上升沿计次模式
	...
	TIM_PWMIConfig(TIM3,&TIM_ICIintSturcture);//更改为交叉通道
